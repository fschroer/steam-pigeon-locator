...
UserInteraction::UserInteraction(FlightManager& flight,
...
void UserInteraction::ProcessChar(uint8_t uart_char, DeviceState& device_state){
  ...
  case UserInteractionState::ConfigHome:
    ...
    case 48: // 0 = Edit deployment channel 1 mode
      ...
      uart_line_len = MakeLine(uart_line_, deploy_mode_edit_text_, num_edit_guidance_text_, DeployModeString(deployment_ch1_mode_));
      break;
    case 49: // 1 = Edit deployment channel 2 mode
      user_interaction_state_ = UserInteractionState::EditDeployChannel2Mode;
      uart_line_len = MakeLine(uart_line_, deploy_mode_edit_text_, num_edit_guidance_text_, DeployModeString(deployment_ch2_mode_));
      break;
    case 50: // 2 = Edit deployment channel 3 mode
      user_interaction_state_ = UserInteractionState::EditDeployChannel3Mode;
      uart_line_len = MakeLine(uart_line_, deploy_mode_edit_text_, num_edit_guidance_text_, DeployModeString(deployment_ch3_mode_));
      break;
    case 51: // 3 = Edit deployment channel 4 mode
      user_interaction_state_ = UserInteractionState::EditDeployChannel3Mode;
      uart_line_len = MakeLine(uart_line_, deploy_mode_edit_text_, num_edit_guidance_text_, DeployModeString(deployment_ch4_mode_));
      break;
...
//    if (valid_record){
      ...
      FormatFloat(apogee_str, apogee, 1);
      uint32_t apogee_timestamp = 0;
      char apogee_timestamp_str[7];
      archive_.ReadEvent(i, FlightArchive::ExampleStatId::ApogeeTimestampMs, apogee_timestamp, valid_record);
      FormatUnixUtc(apogee_timestamp_str, apogee_timestamp);
      uart_line_len = MakeLine(uart_line_, archive_position, datetime, " ");
      uart_line_len += MakeLine(uart_line_ + uart_line_len, apogee_str, "           ") - strlen(apogee_str);
      uart_line_len += MakeLine(uart_line_ + uart_line_len, apogee_timestamp_str);
...
void UserInteraction::AdjustConfigNumericSetting(uint8_t uart_char, int *config_mode_setting, int max_setting_value, bool tenths){
...
void UserInteraction::ExportData(uint16_t archive_position){
  ...
  bool success = archive_.GetFlightSampleCount(archive_position, sample_count_out);
	if (success) {
		FlightArchive::ExampleFlightSample sample_buffer[sample_count_out] {};
		...
		if (archive_.ReadFlightData(archive_position, sample_buffer, sample_count_out, samples_read_out)) {
			for (uint32_t i = 0; i < samples_read_out; i++) {
				export_line.Clear();
				export_line.WriteMany(sample_buffer[i].timestamp_ms, ",", sample_buffer[i].altitude_m, ",",
						sample_buffer[i].accel.x, ",", sample_buffer[i].accel.y, ",", sample_buffer[i].accel.z, ",",
						sample_buffer[i].gyro.x, ",", sample_buffer[i].gyro.y, ",", sample_buffer[i].gyro.z, ",",
						sample_buffer[i].lat_rad, ",", sample_buffer[i].lon_rad, crlf_);
				export_line.Flush();
			}
		}
	}
...
void UserInteraction::ExportFlightStats(uint16_t archive_position){ //Export flight statistics
  ...
  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::DroguePrimaryDeployTimestampMs, time_ms, present_out);
	export_line.Clear();
	export_line.WriteMany(drogue_primary_deploy_sample_index_text, time_ms, crlf_);
	export_line.Flush();

  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::DrogueBackupDeployTimestampMs, time_ms, present_out);
	export_line.Clear();
	export_line.WriteMany(drogue_backup_deploy_sample_index_text, time_ms, crlf_);
	export_line.Flush();

  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::DrogueVelocityThresholdTimestampMs, time_ms, present_out);
	export_line.Clear();
	export_line.WriteMany("Drogue velocity threshold time: ", time_ms, crlf_);
	export_line.Flush();

  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::MainPrimaryDeployTimestampMs, time_ms, present_out);
	export_line.Clear();
	export_line.WriteMany(main_primary_deploy_sample_index_text, time_ms, crlf_);
	export_line.Flush();

  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::MainBackupDeployTimestampMs, time_ms, present_out);
	export_line.Clear();
	export_line.WriteMany(main_backup_deploy_sample_index_text, time_ms, crlf_);
	export_line.Flush();

  archive_.ReadEvent(archive_position, FlightArchive::ExampleStatId::MainVelocityThresholdTimestampMs, time_ms, present_out);
	export_line.Clear();
