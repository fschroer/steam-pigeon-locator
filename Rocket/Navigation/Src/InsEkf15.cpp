...
void InsEkf15::predict(const ImuSample& imu, float dt_s) {
    ...
    // is non-finite or implausibly large, reset the velocity states to 0 and
    // re-inflate the velocity covariance block (states 3-5) so the filter recovers.
    {
        constexpr float kVelDivergenceMps = 1500.0f;   // >> any model-rocket speed
        constexpr float kVelResetVar      = 100.0f;    // (10 m/s)^2 reset uncertainty
        ...
        if (!std::isfinite(vmag2) || vmag2 > kVelDivergenceMps * kVelDivergenceMps) {
            ...
            for (int i = 3; i <= 5; ++i) {
                for (int c = 0; c < n; ++c) { P[i*n + c] = 0.0f; P[c*n + i] = 0.0f; }
                P[i*n + i] = kVelResetVar;
            }
