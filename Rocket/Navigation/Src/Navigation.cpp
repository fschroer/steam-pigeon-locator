...
Navigation::Navigation(SPI_HandleTypeDef* hspi2,
    ...
    : m_imu(hspi2, imu_cs_port, imu_cs_pin),
      m_baro(hspi2, baro_cs_port, baro_cs_pin),
      m_gps(hi2c2, 0x42) {}
...
bool Navigation::Init(const uint16_t output_rate_hz) {
	if (output_rate_hz < 20.0f) m_cfg.output_rate_hz = 20.0f;
	else if (output_rate_hz > 100.0f) m_cfg.output_rate_hz = 100.0f;
	...
	timeout = 5;
	while (!(baro_read_ok = m_baro.readSample(baro)) && timeout-- > 0) {
  ...
  volatile bool init_success = imu_read_ok &&
                               imuStat.initialized &&
                               baroStat.initialized &&
                               gpsStat.initialized;
  (void)init_success;
...
bool Navigation::IsLaunched() {
	...
	const bool threshold = (accel_g >= m_cfg.launch_detect_accel_g) || (gyro_dps >= m_cfg.launch_detect_gyro_dps
			|| m_baro.raw().altitude_m_agl >= m_cfg.launch_detect_agl);
