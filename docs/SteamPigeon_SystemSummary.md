...
- **`FlightSample` is size-locked at 80 bytes, and `ARCHIVE_VERSION` is not bumped for additive fields.** Growing the struct to 84 B adds ~375 KB of chunk stride across the 10-record region against ~200 KB of headroom, and `ValidateHeaderForConfig()` compares `version` and `statsSlotCount` exactly — so bumping either **orphans every record already on the device**. New per-sample data therefore goes into reserved space at unchanged offsets: as of 2026-08-03 the former `_pad[1]` at offset 79 is `gps_fix_sv` (GPS fix type + satellite count + stale-stream classification, [ADR-0017](adr/0017-gps-receiver-configuration-ownership.md)); records written earlier decode as `fix_type 0 / num_sv 0`. Adding a `Statistic` enum entry has the same orphaning effect and is subject to the same rule.
