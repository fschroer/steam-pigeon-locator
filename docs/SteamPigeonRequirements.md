...
The original outline's ranked goals were captured as `FR-P1…FR-P12` with the ID number matching priority. As of v2.1 that coupling is retired (see status key): the **`Pri` column is the contract** for resolving trade-offs (P1 highest), IDs are stable labels, and the table is sorted by `Pri` with *Deferred* items last. `FR-P13` (air starts) was inserted at Pri 3; `FR-P14` (connection authorization) at Pri 7; `FR-P15` (interference detection and channel management) at Pri 8.
...
| **FR-P15** | 8 | Active | **Interference detection and channel management.** Tell the user when the locator↔receiver channel is degraded and why, distinguishing contention from distance, and provide a means to find and move to a clearer channel. A contested channel costs position updates, so this protects delivery of FR-P2 and FR-P5 rather than adding a capability of its own — which is why it sits beside connection authorization: both exist because a launch has many users sharing one band. Rationale, and the correction that co-channel interference *displaces* rather than corrupts (so it is detected by identifying the other locator, not by measuring power), in [ADR-0019](adr/0019-channel-interference-detection.md). |
| **FR-P6** | 9 | Active | Produce locator sound for power-on, arming/disarming, and as a post-landing location aid. |
| **FR-P7** | 10 | Active | Report G-forces on the rocket during flight. |
| **FR-P10** | 11 | Active | Report rocket rotation during flight. |
| **FR-P11** | 12 | Active | Capture general-interest flight data: motor burnout, deployment-channel continuity before and after deployment, and physical parachute-deployment sensing. |
| **FR-P12** | 13 | Active | App text-to-speech for locator status, so the user need not look at the screen. |
...
- FR-L4 — Archive flight data to external flash for later download, including the ~2 s of pre-launch data leading up to launch, timestamped on a GPS-disciplined real-time clock ([ADR-0007](adr/0007-prelaunch-ring-monotonic-clock.md)). Archived data must survive an MCU-only reset (debugger/flash/watchdog) and a flight's committed samples must remain recoverable even if the flight was not cleanly closed (power lost / landing not detected) ([ADR-0010](adr/0010-archive-flash-robustness.md)). The locator must be re-flyable after a landing without a power cycle: a disarm→arm cycle resets the flight state machine and per-flight state and opens a new record ([ADR-0007](adr/0007-prelaunch-ring-monotonic-clock.md) Decision 4, [ADR-0010](adr/0010-archive-flash-robustness.md)).
...
- FR-L6 — Broadcast a reasonably-unique locator ID and authenticate its broadcasts with a connection password that can be set **only** over USB-C (never over the air); act only on commands addressed to that ID (FR-P14, [ADR-0006](adr/0006-locator-connect-password.md), [ADR-0020](adr/0020-targeted-locator-commands.md)).
