...
⚠️ **The set is 16, not 17.** `Ken_Locator_2026-09-05_100815.csv` and `Mike_5_Locator_2026-09-05_100815.csv` are **byte-identical** (md5 `c5ddb9a7c5f7`) — one export saved twice. The analysis doc carries md5s for all of them, deliberately: the 2026-08-31 sweep's numbers cannot be re-derived from the repo because the files were never identified, and ADR-0032's own revisit condition is stuck behind that.

✅ **[ADR-0032](adr/0032-baro-outlier-filtering.md) — the pyro-shock half is CLOSED.** That ADR ended on *"the first flight with charges is the test that closes it."* Sixteen flights with live charges: **zero outlier events**, against a 2026 archive average of ~4.6 per flight. Per-sample noise **σ ≈ 0.04–0.07 m**. The velocity channel read **376 m/s** with no plateau where the removed clamp would have reported `200.0`.

⚠️ **But the descent noise it sized N against is largely AERODYNAMIC, and no window width removes it.** Under drogue the altitude carries a **0.2–0.9 Hz oscillation** varying **50× between airframes** — residual sd 0.11 m on `Mike_8 09-06` vs **7.32 m, max 15.34 m** on `Mike_6 133510`. On `Ken_6` it is **phase-locked to the tilt oscillation** at the same ~1.7 s period, identifying it as the pressure port coupling to attitude. A rank filter cannot reject a sustained oscillation. **The gate, not the filter, is where this belongs**: with ±15 m against a 130 m main gate, require N consecutive samples rather than adding a smoother whose lag scales with the descent rate that makes an early main dangerous.

✅ **[ADR-0017](adr/0017-gps-receiver-configuration-ownership.md) — AIR4 is VALIDATED and is no longer the limiter.** **128 of 28 689 samples (0.45 %)** without a 3D fix; fix **held to 376 m/s**. The longest outage, 3.90 s, was at **11–47 m/s** near apogee — not a model limit. Exactly one high-speed loss anywhere (0.80 s on `Ken_6`). **Zero** stale codes 6/7 in a whole meet, so the config watchdog never had to fire either. The "revisit if fix loss with the airborne model active" condition is answered **in the negative**.

⚠️ **`fix_type` is the wrong field to have archived as the trust signal.** It reads 3 on **99.55 %** of all samples and is constant in 12 of 16 records. On `Ken 132857` it reported a continuous 3D fix from 4.8–11.0 s on **5 satellites with h_acc climbing to 299.6 m**, while `gps_vel_d` read **+101 m/s (descending) as the rocket climbed at 104 m/s**. Only `num_sv` and `gps_h_acc_m` saw it. **This settles the ADR's open item on getting fix quality to the app: send `h_acc`, not `fix_type`.**

🔧 **Three defects opened.** All three are in code paths the ADRs describe correctly and the implementation does not:

- **[#43](https://github.com/fschroer/steam-pigeon-locator/issues/43) — `accel_alt_*` is archived in the SENSOR frame while `accel_*` is in the BODY frame.** `BuildSample` takes one from `nav->body_accel_mps2` (remapped) and the other from `getRawImu()` (not). Fitting `accel_i = s·accel_alt_j` recovers **exactly two** mappings across 16 records — 9 give `−sX,+sY,−sZ` (the 180°-about-Y row of the mounting table), 7 give identity, **nothing in between**. [ADR-0004](adr/0004-fusion-vetting-method.md)'s cross-channel check cannot work until this is fixed. `accel_source` is unaffected.
- **[#44](https://github.com/fschroer/steam-pigeon-locator/issues/44) — the strapdown's accel tilt correction NEVER runs in flight.** `quasi_static` ANDs in `m_gps_velocity_enabled_`, which is true in every state but `WaitingLaunch`/`Landed` and has nothing to do with gravity observability; and `IsStationary`'s `|ω| ≤ 5 dps` passes on **0.0–4.6 %** of descent samples. Meanwhile `|a|−1g` is inside ±0.15 g on **57–97 %** of them. Descent tilt is wrong by a **median of −63° to −141°**, with single-sample steps to **125° in 50 ms**. [ADR-0005 amended](adr/0005-retire-ekf-raw-primary.md) — this needs a decision (fix the code, or narrow the claim), not just a patch.
- **[#45](https://github.com/fschroer/steam-pigeon-locator/issues/45) — `DetectBurnout` tests a MAGNITUDE, so post-burnout drag holds it off.** Both fast flights declared burnout **3.8 s late** (`Ken 132857` motor stop ≈3.0 s, declared 6.797 s; `Ken_6` ≈2.1 s, declared 5.899 s). The axial component changes **sign** at burnout and is unambiguous. `kApogeeMaxThrustG = 1.3 g` has the same blind spot — it reads 4.3 g of *drag* as thrust. Wants a `Tests/FlightReplay` pass before it flies.

📋 **`ekf_health` is right about altitude and blind to velocity.** `fused_frozen` did its job — two records had the fused altitude dead (`peak 0.0 m` against a real 290.7 m) and carried the flag on 1 735 of 1 797 rows. But **135 rows across 10 records** have `|fused_vspeed − raw| > 50 m/s` and **70 of them read `ekf_health = 0`** — including **+910 m/s** in `Ken_6`'s boost and −403/−671/+896 m/s **while sitting on the ground**. Suggested: a fifth bit `0x10 fused_vspeed_implausible`, same symptom-not-mechanism shape that made `fused_frozen` work — bit `0x10` is free inside the existing byte, so **no `ARCHIVE_VERSION` bump** (which would orphan every stored record). **[#46](https://github.com/fschroer/steam-pigeon-locator/issues/46).**

📋 **Two numbers worth carrying forward.** The 2026-09-06 180444 pair (`Mike_5` + `Mike_8`, one airframe, pads 1.8 m apart) reported peak AGL **290.7 m vs 290.4 m** — **0.3 m** is the instrument-to-instrument figure for the whole baro chain. Their two independent strapdowns agreed to **~3° at burnout and ~4° at the coast peak** — that is the measured drift figure a tilt freshness budget should use instead of an assumed one.

⚠️ **Open ends, no issues yet.** (a) The 2026-09-07 110552 pair started 0.7 m apart and landed **150 m apart** with peaks 19.9 m apart, and `Mike_9` **never fired main primary** (`Main primary time: 0`) — two airframes, or one that separated? Needs the flight card. (b) Two locators (`Ken 132857`, `Ken_6`) show the boost thrust vector **38.8°/144.5° off body X** and want a bench mounting-frame check — distinct from #43. (c) The gyro rail is closer than assumed: FS ±4 587 dps, `Ken_6`'s **median** boost rate 2 152 dps, two records railed. (d) **[#47](https://github.com/fschroer/steam-pigeon-locator/issues/47)** — `AnchorRecordToLaunchOnset` keeps one pre-onset sample, so **the tilt seed, gyro bias and baro zero are not in the archive**, exactly what the attitude findings need. The ring already holds 2 s and the `health_flagged` branch already moves the epoch back; what is missing is the decision to stop discarding, since t = 0 would stop meaning thrust onset. (e) **[#48](https://github.com/fschroer/steam-pigeon-locator/issues/48)** — `armed` is constant in **16/16** records, `pps_status` in 14/16, `fix_type` in 12/16. Bigger than it looks: `FlightSample` is 88 B and `ArchiveTypes.hpp` documents the binding constraint as **samples per chunk** (≤ 82 B → 6/chunk → **10** records; ≤ 90 B → 5 → 9), and `record_count` was cut 10 → 9 to fund ARCHIVE_VERSION 6. Dropping `armed` + `pps_status` lands at exactly 82 B. **`fix_type` should stay per-sample** — its job is the 6/7 stale classification, i.e. *which rows are latched*, which a per-flight minimum destroys.
...
**`ChannelMoveRunner` now holds the sequence** — how many times it looks, what it may move,
in what order — which is where the rest of ADR-0011's defects were and which no test could
reach while it sat in `RocketViewModel` with a `BluetoothService` in scope.

Every side effect goes through an `Ops` interface; the runner holds no state, no timing, no
...
plus 374 green tests, and — **✅ 2026-08-30 (fschroer)** — by re-running criterion 2 on
hardware against this build.
...
**`ChannelMoveRunnerTest` (17) drives it against a scripted fake and asserts the exact call
order**, covering the four sequencing defects the bench found: silence asked for twice; a
refusal re-asked once and twice-declined not asked a third time (`NotChecked` must not
inherit `NoEvidence`'s extra look); a lost retry looked at again with the receiver put back
so a failed move never ends split (the ~1-in-8); and the config never re-sent more than once.

**374 tests, 29 suites, 0 failures** (was 357/28). App `0e4ab36`, iOS brief `bc1adfe`.

**What is still uncovered, stated rather than implied:** `channelMoveOps` itself — the
service calls, the 100 ms relink poll, the search plumbing. It is a thin adapter now rather
than the whole sequence, but it is not nothing, and it is the remaining place a defect can
hide from the test suite.

💡 **A note on the pre-commit secret scan.** It matched on this session's diff and the match
was prose: `sk\.` fires on any sentence ending in "…ask." The scan is still worth running —
it caught a real Mapbox token once — but a hit needs reading, not reacting to.

## 2026-08-30 (close-out) — #20 CLOSED; coverage narrowed; the docs gate learned the lesson

**[#20](https://github.com/fschroer/steam-pigeon-locator/issues/20) has no open acceptance
criteria.** 1, 2, 3, 5 pass on hardware; 4 retired as unreachable. The body was rewritten:
criteria 1-3 were still unchecked despite passing, criterion 5 existed only in a comment, and
an earlier edit of mine had corrupted the em-dashes to `^W` through a shell pipeline. All
fixed, and the body now records what the validation cost as well as what it proved.

**The count worth keeping: six defects, all introduced by the amendment, none by the
firmware.** The hung confirm window (a lost locator), the ~1-in-8 retry split, the
refused-probe-read-as-silence, and **three messages that asserted hardware states nobody had
checked**.

**Coverage narrowed, not closed** (app `6a9a6eb`, 341 → **357 tests, 28 suites**). Three
decision points lifted out of the coroutine into pure functions and pinned: `action` (only an
evidenced `LocatorStayed` moves anything), `confirmDeadline` (the regression test drives 50
receipts at the real 2 s poll interval and asserts the ceiling still holds), `relinked` (the
frame test is the load-bearing half), and `message` (which sentence is earned, decided in one
...
hardware passes moved the design four times. Now in dependency order, with the three message
defects in their own section, and instructions to port `ChannelMove` **and both test files
first**, since the tests are what stop iOS repeating the sequence.

## 2026-08-30 (bench 4) — criterion 5 PASSES; the message named the wrong channel — APP FIXED, NOT RE-RUN

**Criterion 5 passes.** *"Could not confirm where the locator is"*, receiver left on the
**old** channel, and powering the locator up resumed the link with no user action.

**Why the OLD channel and not the new one, which is the part worth understanding.** With
the locator already silent, no forwarding window ever opens — it needs a recent
`PreLaunchData` — so the forward never transmits and **the receiver never follows**. That
is [ADR-0011](adr/0011-locator-lora-channel-from-app.md)'s case 0, and it resolves through
`NoEvidence` doing nothing, which is correct: nothing moved, so nothing needs moving back.
The ADR previously described case 0 as resolving by hearing the locator on the old channel;
that is the *late* locator. A **silent** one takes this second route, now recorded.

⚠️ **The verdict was right and the sentence was not.** The message read *"The receiver is
on channel N"* naming the **attempted** channel — false, since the receiver never left the
old one. **That is the second time in this amendment a message asserted a hardware state it
had not checked** (the first was *"left on its previous channel"* on the `NoEvidence` path).
Both no-answer messages now report the receiver's **actual** channel, which the app knows
regardless of firmware version because the channel watch polls `ReceiverInfo` every 2 s
while the locator is quiet — no reliance on the transmit receipt, whose absence stays
...
App `fe5064c`. #20's remaining criteria: 1, 2, 3, 5 pass; 4 retired.

## 2026-08-30 (bench 3) — criterion 4 RETIRED; criterion 5 was blocked by our own queued command — APP FIXED, NOT RE-RUN
...
its supervision timeout, so the write is queued and the command is recorded *Sent*), one
claiming a ~5 s window that does not exist, from confusing `locatorConnected` with
`hearingLocator`, the 5 s broadcast rule that gates *Find a clean channel*. **The gating
closed the exposure structurally**, so the `SendFailure` branch is defensive rather than
load-bearing. Retired in the issue body, not carried as blocked.

⚠️ **Criterion 5 exposed a real defect, and it arrives by construction.** The obvious way
to stage it — move, then power the locator off — makes the receiver **refuse** the probe
rather than run it, and the app read the refusal as *heard nothing*. The move queues a
`LocatorCfgChgRequest`; `IsOperatorCommand` counts it as an operator command; the receiver
refuses a `LocatorSearchRequest` while one is pending; and with the locator silent the
forward can never leave, because its window needs a recent `PreLaunchData`. **So the
undelivered command blocks the very probe that would explain why it was undeliverable**,
until `kPendingTxStaleMs` (10 s) drops it — and the first probe lands ~5 s in, with the
immediate re-probe inside it too.

**That is the fifth instance of ADR-0029's rule** — *a rule about the link must be able to
tell a gap the app created from a gap the world created* — and the first that is
structural rather than unlucky. The 2026-08-29/30 session predicted a fourth and got a
fifth. Fixed: a refusal returns a distinct **`NotChecked`** verdict, is re-asked once after
`CHANNEL_PROBE_REFUSED_RETRY_MS` (6 s, sized against the receiver's stale-drop — a
deliberate coupling noted in both places), and is reported as *could not **check***, not
*could not **confirm***. Two different worlds, one flight line.

**Also: the outcome of a ~23 s cycle was legible for 2 s.** Reported as *"a very quick
message at the top indicating an issue, but it went away before I could read it"* —
`_locatorConfigMessageState` returns to Idle 2 s after the terminal state and the banner
read it directly. Now held in `channelMoveResult` until Dismiss or the next move. The same
edit fixes a regression I introduced with the search-candidate memory: clearing
`_pendingChannelMove` on success also took *"Now on channel N"* off screen in the same
instant, because the banner was keyed on it. The banner has its own channel now.

**The "The receiver is busy…" note under *Search N channels* is the same refusal** on the
manual path, where pressing Search again is the retry. Not a separate bug.

App `ec6ff50`. **Nothing re-measured** — criterion 5 has not been re-run since the fix, and
its two valid outcomes now say different things (*could not confirm* = probe ran and heard
nothing, the criterion proper; *could not check* = refused twice, meaning the retry did not
outlast the blocker, and the receiver's `[search]` trace says what did).

## 2026-08-30 (bench 2) — criterion 2 PASSES; the retry was exempt from its own fix — RECEIVER/LOCATOR UNCHANGED, APP FIXED, NOT RE-RUN

Second hardware pass after `99f4d91`/`9edb947`. **The observed sequence matches the design
exactly**: banner, receiver moves to the new channel, a few seconds of nothing, the
two-channel search, one hit for the locator on the **old** channel, receiver briefly back to
old, then both to new. **Criterion 2 passes.**

⚠️ **~1 run in 8 still ended split**, and the cause was structural rather than marginal.
**The retried `LocatorCfgChgRequest` is a single unacknowledged frame on the channel being
left, and the receiver follows it whether or not the locator hears it** — so losing it
reproduces the original split one layer down. `recoverLocatorChannel` returned at exactly
that point. The amendment fixed the first attempt and left its own retry exempt from the
rule it had just established.

**Fixed by giving the retry the same treatment**: its timeout is followed by one more probe,
which buys an invariant worth carrying — *a failed move never ends with the receiver on a
channel the probe did not confirm.* `LocatorStayed` puts the receiver back so the run ends
together rather than split. Bounded: the probe cannot recurse and there is no second retry.
App `e399104`.

**And silence is now asked for twice.** A `NoEvidence` verdict re-probes once before being
accepted — ADR-0029's *zero frames proves nothing* applies directly, since one 1.4 s dwell
can miss a 1 Hz burst, and `NoEvidence` is the branch that ends with the receiver on a
channel nothing has been heard on. Worst-case failure is now **~23 s** tap to verdict.

**The second candidate cause was not ruled out and does not need to be.** A 1-in-8 split
could also have been a probe that missed the locator's burst (`NoEvidence` → receiver stays
on new). The two are distinguishable by whether the search showed a hit, which was not
recorded. Both are now addressed — the retry probe and the NoEvidence re-probe — so the
next run does not have to answer the question first. **If a split still occurs, note whether
the search rows showed a hit**; that is the discriminator.

**No unit coverage for any of this orchestration** — it is coroutine- and service-bound, and
only `ChannelMove.verdict` is pinned. That gap is why these three defects were all found by
hand on the bench rather than by a test.

## 2026-08-30 (bench) — #20 run on hardware: criteria 1 and 3 PASS, and criterion 2 found a hang that loses the locator — FIXED, UNCOMMITTED-CLEAN, NOT RE-RUN

First hardware exercise of ADR-0011's amendment, via the `SP_LOSS_INJECT` `&` harness.
**Criteria 1 and 3 pass.** Criterion 2 gave three different outcomes across three runs,
and the cause was mine.

**The defect, and it is the one the amendment exists to prevent.** The app re-based its
confirm window on **every** `ReceiverInfo` whose channel matched the move in flight — but
`ReceiverInfo` is not only the receiver's unsolicited transmit receipt. `channelWatchJob`
polls for one **every 2 s** once the locator has been silent for 5 s, which is exactly the
state an unconfirmed move is in. Every reply matched and pushed the deadline out 5 s.
**2 s < 5 s, so the window never closed**: banner stuck on *"Moving to channel N…"*, probe
never ran, nothing resolved, receiver left on the new channel with the locator on the old
one. **A lost locator, reintroduced by the fix for lost locators.**

Fixed twice over, deliberately: the receipt is **latched** (first match only), and the wait
carries an **absolute ceiling** of `2 × CONFIG_CONFIRM_WINDOW_MS` so no future re-base
source can hang it again. App `99f4d91`.

**The rule this is a fresh instance of** — the one ADR-0029 already named and the
2026-08-29/30 session recorded as generating a fourth: *a rule about the link must be able
to tell a gap the app created from a gap the world created.* Here it is one message type
...
opposite places. Now split, via a `channelMoveOutcome` flow. **The docs gate missed this
because I checked the manual and the summary and never opened `strings.xml`** — worth
adding to the gate's Gate-2 checklist.

**Observability was the reason run 1 was unfalsifiable.** The `&` harness announced
*arming* and said nothing when the drop *fired*, so a run whose one-shot had been consumed
earlier looked identical to one where recovery simply worked. The locator now prints
`DIAG|LOSS: LocatorCfgChgRequest DROPPED (forced miss fired)`, from `Factory`'s main loop
rather than the radio RX path.

⚠️ **`NoEvidence` is a coin-flip, and ADR-0011 now says so.** Staying on the new channel is
harmless only if the locator actually moved; if it never moved and the probe missed it,
staying loses it exactly as reverting would lose a locator that did move. The two readings
are symmetric. Staying is kept as the tiebreak — a locator that moved is audible on the new
channel at 1 Hz, so hearing nothing weakly argues against case 2 — but it is a **chosen
default under ambiguity, not a safety**, and the amendment's original wording overstated it.
It is now **criterion 5** on #20 and is the least-tested branch.

📋 **[bench-loss-injection.md](bench-loss-injection.md) §#20 is rewritten.** The old step 4
omitted the probe entirely, and step 5 offered *"arm `&` again"* and *"hold the locator
off"* as interchangeable. **They are not** — the first is criterion 3 (both devices end on
the old channel), the second is criterion 5 (receiver stays on the new one). Anyone
following the old text would have recorded one as a failure of the other.

⚠️ **`SP_LOSS_INJECT` is 1 in the working tree and 0 in the commit.** Same standing hazard
as `SP_BENCH_REPLAY` in `Navigation.hpp`: check `git diff` for that line before every
firmware commit. The bench setting was preserved locally after committing.

**Nothing here is re-measured.** Criterion 2 has still not passed; 4 and 5 are untested.
...
`f61768e`. Cross-referenced by hash and by the grep token `locator search dwell 1400`.

⚠️ **Reflash the receiver before trusting any bench result.** There is **no wire change** —
104 / 28 / 39 are untouched, so no coordinated app+receiver flash is needed — but the timing
constants moved and two guards were added, and an older receiver now behaves differently
from what the docs and the app describe. `text 116404` or later.

**The finding worth carrying.** `kSurveyConfirmDwellMs` was 1200 ms, justified in
[ADR-0029](adr/0029-locator-search-candidate-channels.md) by "a locator is on air ~138 ms
once per second". 138 ms is `TelemetryData` — the frame an **armed** locator sends. Both
scans hunt a *disarmed* one, which sends 118-byte `PreLaunchData` at **~200 ms**. A dwell
must contain a whole burst, so it needs period + airtime = exactly 1200 ms: **zero margin**,
and whether a channel caught its locator came down to phase. Now 1400 ms, and a search dwell
ends early on a hit so the longer dwell costs nothing on a busy band. The survey's confirm
phase shared the constant and therefore the defect, where a miss offers a busy channel *as
clean* — and it had **no bench procedure at all**, which is how its own constant came to be
changed without it being measured.

**The rule three separate defects turned out to share**, now stated once in ADR-0029 rather
than as three incident reports: *a rule about the link must be able to tell a gap the app
created from a gap the world created.* The survey section hid its own scan on a silence
rule; the queued-command abort read the app's own version poll as an operator command; the
status panel read a deliberate scan as a lost locator. Expect a fourth; this screen keeps
generating them.

**DECIDED (fschroer): the receiver's armed flag deliberately never expires.** Once the
connected locator is armed, the system locks in and both scans stay refused until it is
disarmed or the receiver power-cycled. It looks like it blocks recovery and does not — a
rocket that armed on your channel is still on your channel, so it is a range problem, and
90 s of sweeping is 90 s not spent hearing it come back. Recorded in ADR-0029 and surfaced
in the user manual as a ⚠️, because a user who hits it needs to know it is intentional.

**[bench-locator-search.md](bench-locator-search.md) is the artifact to read.** All seven
procedures pass on hardware. §5, §6 and §7 are new; §1–§4 were re-run against the new
constants. Two of the runs changed the file rather than ticking it — §2's description of
when widening is offered was wrong, and §4 failed on the status panel before passing — and
§6 caught a fix that was necessary and not sufficient. §5 carries a diagnostic table that
tells the three causes of a missed locator apart.

**Not fixed, deliberately, both recorded in ADR-0029:** a channel's single hit slot can be
taken by a near-field locator from another channel, and `KnownLocator.last_channel` can be
poisoned by the same off-channel capture. Either can still cost you a locator in a search.

➡️ **iOS owes four items**, each written up in `steam-pigeon-ios/docs/UI_PARITY.md` with the
reasoning a port needs: the channel being left reclaiming the connection mid-move, the
status panel reading a scan as a missing locator, the survey section hiding its own scan,
and `ChannelOccupancy`. The iOS changes in `f61768e` are **strings and markdown only and
were NOT COMPILED** — no Mac in that session.

**Unverified on hardware:** both `ChannelOccupancy` changes, the in-flight pick gating,
candidate ordering (not observable below ~14 remembered locators; unit test only), and two
armed-locator branches — a password changed while armed, and a receiver move while the
connected locator is armed.

`Scripts/sp-status.sh` now reports the iOS repo. It had been silently skipping it — with
uncommitted work in it — because it was the one entry without an absolute-path fallback.
...
> **Current heads (2026-08-31): receiver `96afaf3`, app `7472000`, iOS `ccaab73`; locator = *this handoff commit*, above `b0724ea`.** The locator's own head is deliberately not pinned to a hash here — recording it inside the file it names makes the line wrong the moment it is committed, which is how the previous two entries came to be one commit behind. All four repos are **fully pushed**; app `7472000`, receiver `96afaf3` and iOS `ccaab73` are **clean**. ⚠️ **The locator tree is DIRTY on purpose and stays that way:** beyond this handoff edit it carries `#define SP_VACUUM_SIM 1` in `Rocket/Navigation/Inc/Navigation.hpp` — a deliberate local bench setting for the vacuum-chamber harness that must remain **0** in every commit. Same standing hazard as `SP_LOSS_INJECT`; `Scripts/check-bench-flags.sh` fails on it, and `git diff` that line before every commit. `Scripts/sp-status.sh` now reports the iOS repo — it had been silently skipping it, with uncommitted work in it, as the one entry without an absolute-path fallback (`9552846`). The 2026-08-29/30 session is described at the top of this file; nothing from it is outstanding. The locator head is a merge — an iOS parity-matrix commit (`9097547`) had been pushed from elsewhere and was merged rather than rebased, because the locator commits beneath it are referenced by hash from the already-pushed receiver and app commits. The heads below predate this and are kept for the history they carry. Newest work *in that earlier run*: the bench-replay destination fix (`063d54d`) and the `archive_position removal v1` pair (locator `0a1b180` + receiver `3e614d1`). **ADR-0021 is fully implemented across #35, #36 and #37, and all three are CLOSED and hardware-verified as of 2026-08-07** — [#37](https://github.com/fschroer/steam-pigeon-locator/issues/37) with all seven acceptance criteria ticked, [#36](https://github.com/fschroer/steam-pigeon-locator/issues/36) with all six. Newest first: **`SP_BENCH_REPLAY`** (bench harness, no runtime effect), the **pad-alert fixes + bounded snooze** (`pad alert snooze v1`), the **unsigned nose axis** (`nose axis unsigned v2`), **#37 — the disarmed-rocket alert** (Decision 5), **#36 — arming gates pyro only** (Decisions 1/2/4/6), **#35 — explicit arm state on the wire** (Decision 3). Locator `ebc0043` is a docs-only framing correction, `ed989d9` the original docs-only ADR commit. **Requirements v2.4.5.**
>
> 🔧 **`SP_BENCH_REPLAY` (locator `0420d72`, [docs/bench-replay.md](bench-replay.md)) unblocks the rest.** It replays an archived flight through the live state machine on the bench, which is the only way to reach #36's disarmed-record / landing-beacon / ZUPT criteria and #35's "disarmed locator in flight displays DISARMED" — launch detection needs 5 g for 200 ms (ADR-0015) and cannot be faked by hand. Defaults to **0** and must stay 0 in flight builds. **Verifying a flag build needs care:** `make CXXFLAGS+=-DSP_BENCH_REPLAY=1` silently does nothing (CubeIDE rules hard-code their flags), so a "successful" flag-on build may actually be the flag-off build — edit the header default. **Verify against the built ELF, and use string literals, not `nm`:** `grep -a -c -F 'DIAG|REPLAY' Release/Locator.elf` must be non-zero. `arm-none-eabi-nm` for `TestReplay` symbols gives a FALSE NEGATIVE — at `-O2` with a single call site `startTestReplay` inlines away, and the inline accessors never emit symbols at all. This bit on 2026-08-07: the replay appeared dead on the bench because the flashed build was made one minute BEFORE the header was flipped to 1, and `nm` could not tell the two cases apart. **UPDATE 2026-08-22 — `NAV_TEST` is gone and the harness was BROKEN.** The legacy `NAV_TEST` flag (which `SP_BENCH_REPLAY` used to `#define` to borrow the replay implementation) was removed; the replay machinery now lives under `SP_BENCH_REPLAY` alone. Removing it exposed that **`SP_BENCH_REPLAY=1` had not compiled for some time**: the archive migrated `double lat_rad`/`lon_rad` to `int32_t lat_1e7`/`lon_1e7` and `Navigation::injectTestSample` still referenced the old names, so the harness was dead on arrival for anyone who reached for it. Fixed via the existing `FlightArchive::Deg1e7ToRad` helper. To stop this recurring, **[`Scripts/check-bench-flags.sh`](../Scripts/check-bench-flags.sh)** compile-checks all three bench flags (`SP_BENCH_REPLAY`/`SP_FAULT_INJECT`/`SP_LOSS_INJECT`) at their ENABLED setting and asserts each still defaults to 0 — run it before any bench session or commit that touches archive/nav structs. The other two flags were healthy. Measured cost at `-O0`: +3.6 KB flash, +5.7 KB RAM. Beneath that, the previous run is the ADR-0019 / ADR-0020 work described in the 2026-08-06 section above — roughly 18 locator, 16 receiver and 21 app commits on top of `4f2e96a` / `4cfcd89` / `3f50212`.
>
...
- **NFR-9 high-rate gyro path — IMPLEMENTED & BENCH-CONFIRMED at ~480 Hz (GPS/TIM2):** gyro batched into the ISM6HG256X FIFO at 480 Hz; `ISM6HG256X::drainFifoGyro()` reads the FIFO and `Navigation::Update()` integrates each word, decoupled from the slow path (no 480 Hz SPI ISR — respects NFR-4/NFR-5). **Additive & non-breaking:** if the FIFO is unconfigured/empty (or bench replay), it falls back to the single output-register propagate at the loop dt.
