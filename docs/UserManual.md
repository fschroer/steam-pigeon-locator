...
### Why the two scans give up at different times

If a scan gets no answer at all, the app eventually stops waiting and tells you so. The two do it on different clocks, and the difference is worth knowing because it looks arbitrary until you see what each is watching.

- **A channel scan gives up after about fifteen seconds.** It answers once, at the end, so the app has nothing to watch in the meantime — the only thing it can do is set a deadline for the whole sweep. Fifteen seconds is that deadline: comfortably longer than the eight the sweep really takes, so a slow Bluetooth link never trips it.
- **A locator search gives up after about eight seconds of silence** — not eight seconds in total. It reports each channel as it finishes, so the app watches the *gaps* instead, and the clock restarts on every result. This is why a full 64-channel sweep can run for ninety seconds without ever timing out: it is talking the whole way. Eight seconds is several missed reports in a row, which means the receiver has genuinely stopped.

The practical difference: a search that is finding nothing but still working will keep going and keep telling you where it is. One that has actually died says so within eight seconds. A channel scan gives you no progress either way — it is silent until it answers, which is why it needs a deadline rather than a gap timer.

⚠️ **If a scan gives up, the receiver is the first thing to suspect, not the app.** The usual cause is a receiver whose firmware predates the scan you just asked for, and the message says so. See §3.3.
...
⚠️ **Mismatched app and receiver versions are the one pairing that can stop things working outright.** The two talk to each other over Bluetooth in a fixed format, and that format changes occasionally. When it does, an old receiver and a new app cannot complete a channel scan or a locator search — the app waits, then reports that no answer came and suggests updating the receiver’s firmware. **The two waits are different lengths**: about **fifteen seconds** for a channel scan, about **eight** for a locator search. They are not the same kind of timer, which is why they do not match — see *Why the two scans give up at different times* (§2.5). **That message is literal: it means what it says** — but only if the app made you wait for it. In builds before 2026-09-01 both apps could report it when nothing had been sent at all, with a perfectly matched and perfectly healthy receiver, because the press had collided with the app’s own background polling. **On Android that version arrived the *instant* you pressed the button**, so no pause at all is the giveaway. **On iOS it arrived after the usual wait** and looked exactly like the real thing — there the timing tells you nothing. On either, the receiver is not what is wrong: update the app, and see Part 11. The locator is not affected by that pairing; it talks over the radio in its own format.
...
| Locator search or channel scan says *"No response from the receiver"* and the receiver is fine | On builds before 2026-09-01 the press collided with the app’s own background polling of the receiver and the request was never sent — so the message was about a request the receiver never received. That polling only runs while no locator is being heard, which is exactly when you reach for a search, so it landed where it was most confusing. Pressing again usually worked. Update the app. **On Android** that version appeared the instant you pressed, which tells it apart from a genuine no-answer (those make you wait — about fifteen seconds for a channel scan, eight for a locator search; §2.5). **On iOS** it waited too, so only the app version separates them. |
