...
3b. **A third source asks the physics rather than the vendor: total field strength.** *(Added 2026-08-11.)* On two of the three devices measured, no flag is usable at all — a flag pinned at `HIGH`, or one that never fires, is indistinguishable from a healthy compass, so no arrangement of the two can reach the warning there. On the Pixel 3 XL this source is not a fallback, it is the *only* thing between the user and a silently untrustworthy heading.
   ...
   **iOS reads a different sensor for the same number, and getting that wrong cost the whole decision.** *(Added 2026-09-02, from a device report.)* `TYPE_MAGNETIC_FIELD` is Android's **calibrated** magnetometer — the uncalibrated readings live behind `TYPE_MAGNETIC_FIELD_UNCALIBRATED`, a different sensor entirely. The iOS port read `CMMotionManager.startMagnetometerUpdates`, which is the **raw** field: it carries the phone's own hard-iron offset, and on a MagSafe-era iPhone the magnet array alone holds the total above the 70 µT ceiling permanently. The symptom was an ∞ mark pinned **red** — `UNRELIABLE`, so the AR marker was suppressed too — beside a heading the user checked against a landmark and found near-correct. The envelope was working exactly as specified, on a number that was never the Earth's field.

   iOS's calibrated equivalent is `CMDeviceMotion.magneticField`, which is published **only while device motion runs a magnetic or true-north reference frame**. That is a real cost and it belongs in this decision rather than in a code comment: it means the north-referenced fusion runs for the life of the app rather than only where a bearing is needed. It replaces a raw magnetometer stream that was already running continuously, so the magnetometer's own draw is unchanged and what is added is the fusion.

   `.uncalibrated` **votes nothing rather than voting UNRELIABLE.** CoreMotion documents the field values as unusable in that state and delivers zeros, which the gross band would read as a reading far outside the envelope and suppress the bearing for — the same false positive one layer down. This is the existing "a source that has never spoken contributes nothing" rule, applied to a source that speaks only to say it cannot.

   **Not taken, and left here as the open option:** `CMCalibratedMagneticField.accuracy` (`.uncalibrated` / `.low` / `.medium` / `.high`) is the closest thing iOS has to the vendor flags this decision is built around — closer than the `CLHeading.headingAccuracy` degrees the iOS port currently maps into these levels. Adding it as a fourth source is defensible on the same worst-of terms, and was deliberately **not** bundled into a change whose job was to clear a stuck warning: a device that idled at `.low` would trade a permanent red mark for a permanent yellow one, which is the "device idles at LOW" revisit condition below.
...
- **"The calibrated magnetometer" is a different sensor on each platform, and the wrong one looks identical until it is on a phone.** Both platforms expose a raw and a corrected field; the names do not line up, and the corrected one is not the default on either. Reading the raw field produces a number that is stable, plausible-looking, and biased by the device itself — so the failure is not noise but a permanent verdict, on hardware where the heading is fine. It took a user reporting a mark that was "always there, red" next to a heading they had checked against a landmark.
...
**Revisit if:** the iOS `.accuracy` flag is wanted as a fourth source (see the 2026-09-02 addition to Decision 3b), which needs a device where it is expected to *fire* rather than one where it stays quiet; a device has no magnetometer at all, which leaves only flags that may be pinned and no magnitude to fall back on (logged at registration, so it is diagnosable rather than silent); the magnitude test proves noisy in the field and raises prompts nobody can clear; two sources disagree persistently for the same reason; a device idles at `LOW`; or the heading is wrong by a constant angle again, which would point back at the reference frame rather than at calibration.
