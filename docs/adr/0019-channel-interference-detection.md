...
| Coarse survey dwell of 12 ms | the 1 Hz emitter it was hunting, ~79% of the time idle |
...
`confirmed_frames[5]` rides alongside `confirmed_channel[5]`; a channel with a non-zero count is never suggested, whatever its level. The asymmetry is deliberate and load-bearing: **non-zero excludes, zero proves nothing.** The dwell is one broadcast period plus one frame, so a sparser emitter still slips through, and a non-locator device decodes nothing at all — which is exactly what the level measurements remain for. Frames and power answer different questions and neither replaces the other.

**The confirm dwell was 200 ms too short (corrected 2026-08-30).** A decoded frame is this whole test, and a frame only decodes if the dwell contains **all** of it — so the window must exceed the broadcast period *plus* one frame's airtime, not merely the period. `kSurveyConfirmDwellMs` was 1200 ms against a 1000 ms period and a `PreLaunchData` airtime of ~200 ms (118 bytes at SF7/125 kHz/CR 4/5): exactly on the boundary, zero margin. The 138 ms figure this ADR and [ADR-0029](0029-locator-search-candidate-channels.md) both reasoned from is `TelemetryData`'s, the frame an **armed** locator sends — and both scans look for a *disarmed* one. Now 1400 ms, which restores 200 ms of margin; a whole sweep goes ~7 s → ~7.8 s, well inside the 12 s backstop. Found via a locator search that "sometimes missed one of the locators" (ADR-0029, *The dwell was sized against the wrong frame*); the survey's confirm phase shared the constant and therefore the defect, and a confirm dwell that misses is a channel wrongly **offered** as clean — the failure this phase exists to prevent. **Bench-validated 2026-08-30**: ten surveys with a second locator powered on a known channel, never once offered (`bench-locator-search.md` §7, the survey's first procedure — there had been none, which is how the constant moved without the survey being measured).
