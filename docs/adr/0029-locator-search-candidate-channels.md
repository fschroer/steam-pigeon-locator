...
This is **not** ADR-0011's rejected "locator channel as the single source of truth": the receiver-only control survives, as the default, and switching to a different locator already on another channel is still one tap away. What changed is that touching the rocket is now the thing you ask for rather than the thing you pick between.
