...
- **The probe is now a queued writer ([ADR-0033](0033-serialized-ble-writes.md), 2026-09-01).** Its `ReceiverInfoRequest` goes out through the app's outbound write queue like every other message. That matters in both directions: the probe no longer competes with an operator action for the single GATT operation Android allows in flight, and a probe write that is momentarily refused is retried by the transport rather than counted as an unanswered probe. Invariant 2 still counts *unanswered* probes, not unsent ones.
