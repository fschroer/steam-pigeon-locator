...
**5. The app fills the field at the send choke point,** from the connection established in [ADR-0019](0019-channel-interference-detection.md)'s single-holder rule. `BluetoothService` already gates locator-directed sends on being connected; that flag becomes the id itself, so the thing that authorises the send is the same thing that addresses it. They cannot drift apart.
